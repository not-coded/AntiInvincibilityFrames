package net.notcoded.antiiframes.config;
...
public class AntiIFramesConfig {
    ...
    private static class ConfigCarrier {
        ...
        private void setToDefault() {
            ...
            attackExcludedEntities = new String[] { };
    ...
    public static void preInit() {
        File configFile = new File(FabricLoader.getInstance().getConfigDir().toFile(), "antiiframes.cfg");
    ...
    private static void readConfig(File configFile) {
        ...
        if (configFile.exists()) {
            System.out.println("AntiIFrames: Found existing config file. Reading...");
            ...
            try {
                ...
                if (dump.indexOf("attackCancelThreshold") > 0 || dump.indexOf("attackCancelThreshold") < dump.length()) {
                    System.out.println("AntiIFrames: Done reading config files. Here are the loaded settings:");
                    System.out.println("\n" + dump.substring(firstEntryIndex));
                } else {
                    System.out.println("AntiIFrames: Warning, config file may potentially be corrupted. Loading default values.");
                    setToDefault();
                }
            } catch (IOException e) {
                e.printStackTrace();
                System.out.println("AntiIFrames: Caught exception while reading config file. Loading default values.");
                setToDefault();
            } catch (IndexOutOfBoundsException e) {
                System.out.println("AntiIFrames: Config file was potentially corrupted. ");
                setToDefault();
            }
        } else {
            System.out.println("AntiIFrames: Did not found config file. Writing first time config file...");
            ConfigCarrier firstTime = new ConfigCarrier();
            try {
                FileWriter writer = new FileWriter(configFile);
                writer.write(yamlParser.dump(firstTime));
                writer.close();
                System.out.println("AntiIFrames: Done writing first time config file.");
                setToCarrier(firstTime);
            } catch (IOException e) {
                e.printStackTrace();
                System.out.println("AntiIFrames: Caught exception while writing first time config file. Loading default values.");
                setToDefault();
            }
